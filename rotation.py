...
bmp.makeFile(path + image + ".rotated.bmp")

with open(path + image + ".rotated.bmp", "rb") as f:
...
duracion = datetime.datetime.now()
...
for i in range(bmp.start, bmp.start + bmp.width * bmp.height * bmp.bpp, bmp.bpp):
    if bmp.data[i:i+3] != (255, 255, 255):
        ...
        newX = round(round(pixelX - math.tan(angle/2) * pixelY) - math.tan(angle/2) * round(round(pixelX - math.tan(angle/2) * pixelY) * math.sin(angle) + pixelY))
        newY = round(round(pixelX - math.tan(angle/2) * pixelY) * math.sin(angle) + pixelY)
...
duracion = datetime.datetime.now() - duracion

print("duracion: %r ms" % (duracion.microseconds / 1000))

#-------------------------------------------------------------------------------

bmp.makeFile(path + image + ".rotated.bmp")
#bmpRotated.makeFile(path + image + ".rotated.bmp")
#print(path + image + ".rotated.bmp")
